...
def _get_client(api_key: str | None = None):
    """Get a Gemini client using the provided API key or the default one."""
    key = api_key or _default_api_key
    return genai.Client(api_key=key)


def call_llm(prompt: str, retries: int = 3, api_key: str | None = None) -> str | None:
...
def plan_interview(resume: str, jd: str, api_key: str | None = None) -> dict:
    """
...
- Return valid JSON only"""
    ...
    response_text = call_llm(prompt, retries=3, api_key=api_key)
