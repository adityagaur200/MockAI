...
def process_audio_answer(interview_id, file_path):
    ...
    gemini_api_key = interview.get("gemini_api_key")
    ...
    eval_result = evaluator_agent(current_question, transcript, coverage, plan, gemini_api_key)
    ...
    critic_result = critic_agent(eval_result, plan, coverage, compressed, gemini_api_key)
    ...
    if critic_result.get("should_end"):
        ...
        report = reporter_agent(compressed_final, plan, coverage, gemini_api_key)
