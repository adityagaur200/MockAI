...
async def start_interview(user_id, resume_text, job_description, job_name, gemini_api_key=None):
    """
    ...
    plan = plan_interview(resume_text, job_description, gemini_api_key)
    print(f"Plan created for: {plan.get('candidate_name')}")

    # 2. Generate first question using the plan
    first_question = await generate_first_question(resume_text, job_description, plan, gemini_api_key)
